...
export default function UserHistoryYearMonth() {
  ...
  const thisDate = dayjs().year(year).month(month - 1);
  ...
  <>
    <div>
      ...
      <div className="join">
        <Link to={`/user/history/${prevDate.year}/${prevDate.month}`} className="join-item btn btn-sm">
          {`≪ ${parseInt(prevDate.month)}月`}
        </Link>
        <div className="join-item btn btn-sm btn-disabled">
          {`${year}.${month}`}
        </div>
        <Link to={`/user/history/${nextDate.year}/${nextDate.month}`} className="join-item btn btn-sm">
        ...
        <tbody>
          {purchases.map((purchase, index) => (
            ...
            <tr>
              ...
              <td></td>
              <td>合計</td>
              <td>&yen; {purchases.reduce((sum, purchase) => sum + purchase.item.price, 0)}</td>
            </tr>
        </tbody>
      </table>
    </div>
